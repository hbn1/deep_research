...
PROMPTS = {
    ...
    "write": (
        ...
        "输出一份结构清晰、语言流畅、专业易读且"
        "篇幅详实（至少2000-3000字以上）的 Markdown 格式深度研究报告。\n\n"
        "报告应包含：\n"
        "1. 标题（简明扼要，具有洞察力）\n"
        "2. 核心摘要（200字左右，总结最重要的发现）\n"
        "3. 详细分析（这是报告的主体部分，必须极其详实。"
        "请将每个 finding 展开为长篇连贯的段落，进行深度剖析、背景补充和逻辑推演，"
        "严禁一笔带过，并在引用证据时使用上标如 [WEB1_1-1]）\n"
    ...
    "direct_answer": (
        ...
        "直接回答用户，不要走研究报告结构。"
        "要求：简洁、自然、准确。如果用户问天气但未提供城市，请先提示补充城市。"
    ),
